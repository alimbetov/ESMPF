...
class PlatformServiceImpl implements PlatformService {
    ...
    @Override @Transactional public PublicTokenResponse consumePublicToken(UUID id) { PublicAccessToken e = requireToken(id); if (e.getRevokedAt() != null) throw new IllegalStateException("Token revoked"); if (!e.getExpiresAt().isAfter(Instant.now())) throw new IllegalStateException("Token expired"); if (e.getMaxUses() != null && e.getUsedCount() >= e.getMaxUses()) throw new IllegalStateException("Token usage limit reached"); e.setUsedCount(e.getUsedCount() + 1); return response(tokenRepository.saveAndFlush(e)); }
    ...
    @Override @Transactional public DataJobResponse createDataJob(DataJobCommand c) { if (c.sourceAttachmentId() != null) documentReferences.requireAttachment(c.sourceAttachmentId()); DataJob e = new DataJob(); e.setBusinessId(tenant()); e.setType(c.type()); e.setFormat(c.format()); e.setSubjectType(c.subjectType()); e.setSourceAttachmentId(c.sourceAttachmentId()); e.setConfigurationJson(c.configurationJson()); e.setStatus("QUEUED"); e.setProgress(0); return response(dataJobRepository.saveAndFlush(e)); }
    ...
    @Override @Transactional(readOnly = true) public Page<DataJobResponse> listDataJobs(Pageable p) { return dataJobRepository.findAllByBusinessId(tenant(), p).map(PlatformServiceImpl::response); }

    @Override @Transactional public OutboxResponse appendOutboxEvent(OutboxCommand c) { if (c.eventVersion() == null || c.eventVersion() <= 0) throw new IllegalArgumentException("eventVersion must be positive"); OutboxEvent e = new OutboxEvent(); e.setBusinessId(tenant()); e.setAggregateType(c.aggregateType()); e.setAggregateId(c.aggregateId()); e.setEventType(c.eventType()); e.setEventVersion(c.eventVersion()); e.setPayloadJson(c.payloadJson()); e.setStatus("PENDING"); e.setAttemptCount(0); e.setNextAttemptAt(Instant.now()); return response(outboxRepository.saveAndFlush(e)); }
    ...
    @Override @Transactional(readOnly = true) public Page<OutboxResponse> listOutbox(Pageable p) { return outboxRepository.findAllByBusinessId(tenant(), p).map(PlatformServiceImpl::response); }

    @Override @Transactional public AuditResponse appendAudit(AuditCommand c) { AuditLog e = new AuditLog(); e.setBusinessId(tenant()); e.setActorType(c.actorType()); e.setActorId(c.actorId()); e.setAction(c.action()); e.setSubjectType(c.subjectType()); e.setSubjectId(c.subjectId()); e.setBeforeDataJson(c.beforeDataJson()); e.setAfterDataJson(c.afterDataJson()); e.setMetadataJson(c.metadataJson()); e.setOccurredAt(Instant.now()); return response(auditRepository.saveAndFlush(e)); }
    @Override @Transactional(readOnly = true) public Page<AuditResponse> listAudit(Pageable p) { return auditRepository.findAllByBusinessId(tenant(), p).map(PlatformServiceImpl::response); }

    @Override @Transactional public IdempotencyResponse beginIdempotentOperation(IdempotencyCommand c) { var existing = idempotencyRepository.findByBusinessIdAndIdempotencyKeyAndOperation(tenant(), c.key(), c.operation()); if (existing.isPresent()) { IdempotencyRecord e = existing.get(); if (!e.getRequestHash().equals(c.requestHash())) throw new IllegalArgumentException("Idempotency key reused with different request"); return response(e); } if (c.expiresAt() == null || !c.expiresAt().isAfter(Instant.now())) throw new IllegalArgumentException("Idempotency expiry must be in future"); IdempotencyRecord e = new IdempotencyRecord(); e.setBusinessId(tenant()); e.setIdempotencyKey(c.key()); e.setOperation(c.operation()); e.setRequestHash(c.requestHash()); e.setStatus("STARTED"); e.setExpiresAt(c.expiresAt()); return response(idempotencyRepository.saveAndFlush(e)); }
    ...
    @Override @Transactional public IntegrationResponse createIntegration(IntegrationCommand c) { if (integrationRepository.existsByBusinessIdAndNameIgnoreCase(tenant(), c.name())) throw new IllegalArgumentException("Integration name already exists"); IntegrationConnection e = new IntegrationConnection(); e.setBusinessId(tenant()); apply(c, e); e.setStatus("INACTIVE"); return response(integrationRepository.saveAndFlush(e)); }
    @Override @Transactional(readOnly = true) public IntegrationResponse getIntegration(UUID id) { return response(requireIntegration(id)); }
    ...
    @Override @Transactional(readOnly = true) public Page<IntegrationResponse> listIntegrations(Pageable p) { return integrationRepository.findAllByBusinessId(tenant(), p).map(PlatformServiceImpl::response); }

    @Override @Transactional public String allocateDocumentNumber(String type, int year, String prefix) { if (year < 2000 || year > 9999) throw new IllegalArgumentException("Invalid sequence year"); DocumentSequence e = sequenceRepository.findByBusinessIdAndDocumentTypeAndSequenceYear(tenant(), type, year).orElseGet(() -> { DocumentSequence n = new DocumentSequence(); n.setBusinessId(tenant()); n.setDocumentType(type); n.setSequenceYear(year); n.setPrefix(prefix); n.setCurrentValue(0L); return n; }); e.setCurrentValue(e.getCurrentValue() + 1); e = sequenceRepository.saveAndFlush(e); String effectivePrefix = e.getPrefix() == null ? "" : e.getPrefix(); return effectivePrefix + year + "-" + String.format("%06d", e.getCurrentValue()); }
