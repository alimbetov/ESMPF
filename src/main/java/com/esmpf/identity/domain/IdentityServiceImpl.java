...
class IdentityServiceImpl implements IdentityService, IdentityReferenceQuery {
    ...
    public BusinessResponse updateCurrentBusiness(BusinessUpdateCommand command) {
        Business entity = requireBusiness(); checkVersion("Business", entity.getId(), command.version(), entity.getVersion());
        mapper.update(command, entity); return mapper.toResponse(businessRepository.saveAndFlush(entity));
    }
    ...
    public BusinessLocationResponse createLocation(BusinessLocationCommand command) {
        ...
        BusinessLocation entity = mapper.toEntity(command); entity.setBusinessId(tenant());
    ...
    public BusinessLocationResponse updateLocation(UUID locationId, BusinessLocationCommand command) {
        BusinessLocation entity = requireLocation(locationId); checkVersion("BusinessLocation", locationId, command.version(), entity.getVersion());
        validateCoordinates(command.latitude(), command.longitude()); mapper.update(command, entity);
        return mapper.toResponse(locationRepository.saveAndFlush(entity));
    }

    @Override @Transactional
    public BusinessLocationResponse deactivateLocation(UUID locationId, long version) {
        BusinessLocation entity = requireLocation(locationId); checkVersion("BusinessLocation", locationId, version, entity.getVersion());
        entity.setActive(false); return mapper.toResponse(locationRepository.saveAndFlush(entity));
    }
    ...
    @Override @Transactional public UserAccountResponse activateUser(UUID userId, long version) { return setUserActive(userId, version, true); }
    @Override @Transactional public UserAccountResponse deactivateUser(UUID userId, long version) { return setUserActive(userId, version, false); }
    ...
    public WorkerQualificationResponse createQualification(WorkerQualificationCommand command) {
        ...
        if (!Boolean.TRUE.equals(user.getWorker())) throw new IllegalArgumentException("Qualification requires a worker user");
        validateDateRange(command.validFrom(), command.validUntil());
        WorkerQualification entity = mapper.toEntity(command); entity.setBusinessId(tenant());
    ...
    public Page<WorkerQualificationResponse> listQualifications(UUID userId, Pageable pageable) {
        requireUserEntity(userId); return qualificationRepository.findAllByBusinessIdAndUserId(tenant(), userId, pageable).map(mapper::toResponse);
    }

    @Override @Transactional
    public WorkerQualificationResponse updateQualification(UUID qualificationId, WorkerQualificationCommand command) {
        WorkerQualification entity = requireQualification(qualificationId); checkVersion("WorkerQualification", qualificationId, command.version(), entity.getVersion());
        if (command.userId() != null && !command.userId().equals(entity.getUserId())) throw new IllegalArgumentException("Qualification owner cannot be changed");
        validateDateRange(command.validFrom(), command.validUntil()); mapper.update(command, entity);
        return mapper.toResponse(qualificationRepository.saveAndFlush(entity));
    }

    @Override @Transactional
    public WorkerQualificationResponse expireQualification(UUID qualificationId, long version) {
        WorkerQualification entity = requireQualification(qualificationId); checkVersion("WorkerQualification", qualificationId, version, entity.getVersion());
        entity.setStatus("EXPIRED"); return mapper.toResponse(qualificationRepository.saveAndFlush(entity));
    }
    ...
    private BusinessResponse transitionBusiness(long version, String status) {
        Business entity = requireBusiness(); checkVersion("Business", entity.getId(), version, entity.getVersion()); entity.setStatus(status);
    ...
    private UserAccountResponse setUserActive(UUID id, long version, boolean active) {
        UserAccount entity = requireUserEntity(id); checkVersion("UserAccount", id, version, entity.getVersion()); entity.setActive(active);
        return mapper.toResponse(userRepository.saveAndFlush(entity));
    }
    private Business requireBusiness() { return businessRepository.findById(tenant()).orElseThrow(() -> new EntityNotFoundException("Business", tenant())); }
    private BusinessLocation requireLocation(UUID id) { return locationRepository.findByIdAndBusinessId(id, tenant()).orElseThrow(() -> new EntityNotFoundException("BusinessLocation", id)); }
    ...
    private static void validateCoordinates(Double latitude, Double longitude) {
        ...
        if (latitude != null && (latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180)) throw new IllegalArgumentException("Invalid coordinates");
    }
    private static void validateDateRange(java.time.LocalDate from, java.time.LocalDate until) { if (from != null && until != null && until.isBefore(from)) throw new IllegalArgumentException("validUntil must not be before validFrom"); }
