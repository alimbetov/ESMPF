...
class ServiceSupportServiceImpl implements ServiceSupportService {
    ...
    @Override @Transactional(readOnly = true) public Page<RecommendationResponse> listRecommendations(UUID equipmentId, Pageable p) { equipmentReferences.requireEquipment(equipmentId); return recommendationRepository.findAllByBusinessIdAndEquipmentId(tenant(), equipmentId, p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<MaterialCatalogResponse> listMaterials(Pageable p) { return materialRepository.findAllByBusinessId(tenant(), p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<JobMaterialResponse> listJobMaterials(UUID jobId, Pageable p) { serviceReferences.requireJob(jobId); return jobMaterialRepository.findAllByBusinessIdAndJobId(tenant(), jobId, p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<ServiceAgreementResponse> listAgreements(UUID customerId, Pageable p) { customerReferences.requireCustomer(customerId); return agreementRepository.findAllByBusinessIdAndCustomerId(tenant(), customerId, p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<WarrantyCaseResponse> listWarrantyCases(UUID equipmentId, Pageable p) { equipmentReferences.requireEquipment(equipmentId); return warrantyRepository.findAllByBusinessIdAndEquipmentId(tenant(), equipmentId, p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<MobileDeviceResponse> listDevices(UUID userId, Pageable p) { identityReferences.requireUser(userId); return deviceRepository.findAllByBusinessIdAndUserId(tenant(), userId, p).map(this::response); }
    ...
    @Override @Transactional(readOnly = true) public Page<SyncOperationResponse> listSyncOperations(UUID deviceId, Pageable p) { requireDevice(deviceId); return syncRepository.findAllByBusinessIdAndDeviceId(tenant(), deviceId, p).map(this::response); }
