...
class ServiceSupportServiceImpl implements ServiceSupportService {
    ...
    @Override @Transactional public RecommendationResponse createRecommendation(RecommendationCommand c) { equipmentReferences.requireEquipment(c.equipmentId()); if (c.sourceJobId() != null) serviceReferences.requireJob(c.sourceJobId()); Recommendation e = new Recommendation(); e.setBusinessId(tenant()); e.setEquipmentId(c.equipmentId()); e.setSourceJobId(c.sourceJobId()); e.setDescription(c.description()); e.setPriority(c.priority()); e.setDueDate(c.dueDate()); e.setStatus("OPEN"); return response(recommendationRepository.saveAndFlush(e)); }
    ...
    @Override @Transactional(readOnly = true) public Page<RecommendationResponse> listRecommendations(UUID equipmentId, Pageable p) { equipmentReferences.requireEquipment(equipmentId); return recommendationRepository.findAllByBusinessIdAndEquipmentId(tenant(), equipmentId, p).map(ServiceSupportServiceImpl::response); }

    @Override @Transactional public MaterialCatalogResponse createMaterial(MaterialCatalogCommand c) { validateMaterial(c.defaultPrice(), c.currency()); if (materialRepository.existsByBusinessIdAndCodeIgnoreCase(tenant(), c.code())) throw new IllegalArgumentException("Material code already exists"); MaterialCatalogItem e = new MaterialCatalogItem(); e.setBusinessId(tenant()); apply(c, e); e.setActive(true); return response(materialRepository.saveAndFlush(e)); }
    @Override @Transactional(readOnly = true) public MaterialCatalogResponse getMaterial(UUID id) { return response(requireMaterial(id)); }
    ...
    @Override @Transactional(readOnly = true) public Page<ServiceAgreementResponse> listAgreements(UUID customerId, Pageable p) { customerReferences.requireCustomer(customerId); return agreementRepository.findAllByBusinessIdAndCustomerId(tenant(), customerId, p).map(ServiceSupportServiceImpl::response); }

    @Override @Transactional public WarrantyCaseResponse openWarrantyCase(WarrantyCaseCommand c) { equipmentReferences.requireEquipment(c.equipmentId()); if (c.jobId() != null) serviceReferences.requireJob(c.jobId()); WarrantyCase e = new WarrantyCase(); e.setBusinessId(tenant()); e.setEquipmentId(c.equipmentId()); e.setJobId(c.jobId()); e.setSource(c.source()); e.setDescription(c.description()); e.setStatus("OPEN"); e.setOpenedAt(Instant.now()); return response(warrantyRepository.saveAndFlush(e)); }
    @Override @Transactional(readOnly = true) public WarrantyCaseResponse getWarrantyCase(UUID id) { return response(requireWarranty(id)); }
    ...
    @Override @Transactional(readOnly = true) public Page<WarrantyCaseResponse> listWarrantyCases(UUID equipmentId, Pageable p) { equipmentReferences.requireEquipment(equipmentId); return warrantyRepository.findAllByBusinessIdAndEquipmentId(tenant(), equipmentId, p).map(ServiceSupportServiceImpl::response); }

    @Override @Transactional public MobileDeviceResponse registerDevice(MobileDeviceCommand c) { var user = identityReferences.requireUser(c.userId()); if (!user.active()) throw new IllegalStateException("User is inactive"); if (deviceRepository.existsByBusinessIdAndDeviceIdentifier(tenant(), c.deviceIdentifier())) throw new IllegalArgumentException("Device already registered"); MobileDevice e = new MobileDevice(); e.setBusinessId(tenant()); e.setUserId(c.userId()); e.setDeviceIdentifier(c.deviceIdentifier()); e.setPlatform(c.platform()); e.setAppVersion(c.appVersion()); e.setStatus("ACTIVE"); e.setRegisteredAt(Instant.now()); e.setLastSeenAt(Instant.now()); return response(deviceRepository.saveAndFlush(e)); }
    @Override @Transactional(readOnly = true) public MobileDeviceResponse getDevice(UUID id) { return response(requireDevice(id)); }
    ...
    @Override @Transactional(readOnly = true) public Page<SyncOperationResponse> listSyncOperations(UUID deviceId, Pageable p) { requireDevice(deviceId); return syncRepository.findAllByBusinessIdAndDeviceId(tenant(), deviceId, p).map(ServiceSupportServiceImpl::response); }
